...
function scenarioRowsForCurrentSelection() {
  const family = activeScenarioFamily();
  if (!family) return [];
  if (!scenarioUiState.values || Object.keys(scenarioUiState.values).length === 0) {
    scenarioUiState.values = defaultScenarioValues(family, scenarioUiState.levelId);
  ...
  return family.parameters.map((p) => {
    const base = Number(p.values.as_is) || 0;
    const current = Number.isFinite(scenarioUiState.values[p.key]) ? scenarioUiState.values[p.key] : base;
    const delta = current - base;
    const deltaPct = base === 0 ? 0 : (delta / Math.abs(base)) * 100;
    return { key: p.key, label: p.label, base, current, delta, deltaPct };
  });
}

function scenarioDriverScores(rows) {
  return rows
    .map((r) => ({ ...r, score: Math.abs(r.deltaPct) + Math.abs(r.delta) * 0.1 }))
...
function renderScenarioAttributionPanel(rows) {
  ...
  const msg =
    top.length === 0
      ? "No major scenario deviation from baseline yet."
      : `In ${family.title}, the strongest input levers are ${top.map((t) => prettyDriverLabel(t)).join(", ")}. Most affected outputs: ${outputFocus}.`;
  const risk = top.some((t) => /ChkTime|PhyChk|IdnChk/i.test(t.key))
    ? "Border inspection intensity is elevated; queue and dwell-time risk rises."
    : "No immediate border-intensity spike detected from current scenario settings.";
  const act = top.some((t) => /green/i.test(t.key))
    ? "Keep green-lane ratios high where feasible to reduce red-route pressure."
    : "Prioritise documentary/physical check-time reduction to recover throughput.";
  const panel = el("div", { class: "scenario-llm-panel" },
